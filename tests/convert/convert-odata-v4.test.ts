...
import { convertODataV4EnrichmentToOrd } from "../../src/convert/convert-odata-v4";
import type { ODataV4Enrichment } from "../../src/convert/types";
...
test("descriptionAnnotations omits keys when value is empty string", () => {
	const enrichment: ODataV4Enrichment = {
		protocol: "odatav4",
	...
	const { overlay } = convertODataV4EnrichmentToOrd(enrichment);
	const patch = overlay.patches[0];
	const data = patch.data as Record<string, unknown>;

	assert.ok(
		"@Core.Description" in data,
		"summary should produce @Core.Description",
	);
	assert.ok(
		!("@Core.LongDescription" in data),
		"empty description should not produce @Core.LongDescription key",
	);
});

test("functionImport with matching functions[] entry but different description emits lost-information warning", () => {
	const source: ODataV4Enrichment = {
		protocol: "odatav4",
		namespace: "com.example.Svc",
		summary: "Svc",
		...
		functionImports: [
			{
				name: "GetSummary",
				summary: "Import for GetSummary",
				description: "Exposes GetSummary via container.",
			},
		],
	};
	...
	// No extra patch — import is merged onto the existing function patch
	// patch[0] = namespace, patch[1] = GetSummary
	assert.equal(overlay.patches.length, 2);

	// Op description is preserved — import description loses
	assert.deepEqual(overlay.patches[1].data, {
		"@Core.Description": "Get summary",
		"@Core.LongDescription": "Reads a summary.",
	});
...
test("actionImport tags are merged onto the existing operation patch without duplicates", () => {
	const source: ODataV4Enrichment = {
		protocol: "odatav4",
		namespace: "com.example.Svc",
		summary: "Svc",
		...
		actionImports: [
			{
				...
				summary: "Approve record",
				description: "Approves the record.",
				tags: ["finance", "billing"], // "billing" already on the op
			},
		],
	};
	...
	// patch[0] = namespace, patch[1] = Approve
	const opPatch = overlay.patches[1];
	assert.ok(opPatch);
	// Tags merged, "billing" appears only once
	assert.deepEqual(opPatch.tags?.sort(), ["billing", "finance"]);
});

test("functionImport with no matching functions[] entry generates a patch and needs-spec-extension warning", () => {
	const source: ODataV4Enrichment = {
		protocol: "odatav4",
		namespace: "com.example.Svc",
		summary: "Svc",
	...
	const importPatch = overlay.patches.find(
		(p) =>
			typeof p.selector === "object" &&
			"operation" in p.selector &&
			(p.selector as unknown as Record<string, unknown>).operation ===
				"com.example.Svc.ListItems",
	);
	assert.ok(importPatch, "should generate a patch for the functionImport");
	assert.deepEqual(importPatch?.data, {
		"@Core.Description": "List all items",
		"@Core.LongDescription": "Returns all items.",
	});
	...
	assert.equal(importWarnings.length, 1);
	assert.equal(importWarnings[0].type, "needs-spec-extension");
});

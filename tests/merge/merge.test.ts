...
import { applyOverlayToDocument } from "../../src/merge/merge";
import { type JSONValue, OverlayMergeError } from "../../src/merge/types";
import {
	captureWarnings,
	createOrdOverlay,
	createOverlayPatch,
...
test("applies JSONPath-based merge, update and remove patches to a JSON metadata file", async () => {
	const openApiSource = await loadJsonFixture<JSONValue>(
		"tests/fixtures/targets/astronomy-v1.oas3.json",
	);
...
// ordId selector temporarily removed from spec — restore tests when selector is re-added
// See: src/merge/selectors.ts resolveSelector(), src/merge/validation.ts SelectorKind
test.todo("applies ordId selectors to ORD document resources");
test.todo(
	"derives ORD collection names from ordId resource types ending in 'y'",
);

// ---- commented-out original test bodies preserved below for reference ----
// test("applies ordId selectors to ORD document resources", async () => {
// 	const documentSource = await loadJsonFixture<Record<string, JSONValue>>(
// 		"tests/fixtures/targets/document-1.json",
// 	);
...
// 	assert.equal(
// 		eventResources[0].ordId,
// 		"sap.foo:eventResource:BillingDocumentEvents:v1",
// 	);
// 	assert.deepEqual(merged.customArrayWithoutOrdId, [
// 		{ id: "foo", title: "bar" },
// 	]);
...
test("validates openapi-v3 targets by document content", () => {
	...
	assert.throws(
		() => applyOverlayToDocument(notOpenApiV3, overlay),
		OverlayMergeError,
	);
});
...
test("applies operation selector to OpenAPI operationId", () => {
	...
	assert.equal(
		"deprecated" in (paths["/things"].post as Record<string, unknown>),
		false,
	);
	assert.equal(
		"deprecated" in (paths["/things/{id}"].get as Record<string, unknown>),
		false,
	);
});
...
test("auto-detects OpenAPI operation selector when no definitionType is given", async () => {
	...
	const paths = (merged as Record<string, unknown>).paths as Record<
		string,
		Record<string, Record<string, unknown>>
	>;
	...
	assert.ok(
		warnings.some((message) =>
			message.includes("target.definitionType is RECOMMENDED"),
		),
	);
	assert.ok(
		warnings.some((message) =>
			message.includes('operation" selector works best'),
		),
	);
});
...
test("throws when operation selector is used with an unsupported definitionType", () => {
	...
	assert.throws(
		() =>
			applyOverlayToDocument(source, overlay, {
				context: { definitionType: "edmx" },
				validateDefinitionType: false,
			}),
		(err: Error) => {
			assert.ok(
				err.message.includes("not supported for definitionType"),
...
test("throws when remove targets the root document", () => {
	...
	assert.throws(
		() => applyOverlayToDocument({ keep: true }, overlay),
		/Removing the root document is not supported/,
	);
});
...
test("merge replaces value and warns when incoming type differs from base type", async () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				action: "merge",
	...
	assert.deepEqual(
		(result as unknown as { info: { structured: boolean } }).info,
		{ structured: true },
	);
	assert.ok(warnings.some((w) => w.includes("Type mismatch")));
});

test("merge does not warn when replacing a scalar with the same type", async () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				action: "merge",
				selector: { jsonPath: "$.title" },
				data: "New Title",
			}),
		],
	});
	...
	const { result, warnings } = await captureWarnings(() =>
		applyOverlayToDocument({ title: "Old Title" }, overlay, {
			validateOverlaySemantics: false,
		}),
	);

	assert.deepEqual((result as { title: string }).title, "New Title");
	assert.equal(warnings.filter((w) => w.includes("Type mismatch")).length, 0);
});
...
test("applying the same merge overlay twice is idempotent for scalar properties", () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				selector: { jsonPath: "$.info" },
	...
	const source = { info: { title: "API" } } as JSONValue;
	const once = applyOverlayToDocument(source, overlay, {
		validateOverlaySemantics: false,
	}) as { info: Record<string, unknown> };
	const twice = applyOverlayToDocument(once as JSONValue, overlay, {
		validateOverlaySemantics: false,
		noMatchBehavior: "warn",
	}) as { info: Record<string, unknown> };

	assert.deepEqual(twice.info, { title: "API", version: "2.0.0" });
});

test("remove mask shorter than target array leaves elements beyond mask length untouched", () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				action: "remove",
				selector: { jsonPath: "$.items" },
				data: [{ id: null }],
			}),
		],
	});
	...
	// Mask only touches index 0 (removes .id). Indices 1 and 2 are fully preserved.
...
test("two merge patches on the same selector accumulate changes sequentially", () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				selector: { jsonPath: "$.info" },
	...
	assert.deepEqual(merged.info, { title: "API", x: 1, y: 2 });
});

test("parameter selector patches a named parameter in an OpenAPI operation", () => {
	const source = {
		openapi: "3.0.0",
		paths: {
			"/employees/{id}": {
	...
	const overlay = createOrdOverlay({
		...
		patches: [
			createOverlayPatch({
				...
				data: { description: "The employee ID in UUID format" },
			}),
		],
	});
	...
	const params = (
		(
			(merged.paths as Record<string, unknown>)["/employees/{id}"] as Record<
				string,
				unknown
			>
	...
	assert.ok(idParam);
	assert.equal(idParam.description, "The employee ID in UUID format");

	const formatParam = params.find((p) => p.name === "format");
	assert.ok(formatParam);
	assert.equal("description" in formatParam, false);
});

test("remove then merge replaces an array value entirely", () => {
	const source = {
		info: { title: "API", tags: ["legacy", "internal"] },
	} as JSONValue;
	...
	const overlay = createOrdOverlay({
		patches: [
			...
			createOverlayPatch({
				action: "merge",
				selector: { jsonPath: "$.info" },
				data: { tags: ["public", "stable"] },
			}),
		],
	});

	const merged = applyOverlayToDocument(source, overlay) as {
		info: Record<string, unknown>;
	};
	assert.deepEqual(merged.info.tags, ["public", "stable"]);
});

...
import type { ORDOverlay } from "../../src/spec";
import { applyOverlayToEdmxDocument } from "../../src/merge/edmx";
import { applyOverlayToDocument } from "../../src/merge/merge";
import { OverlayMergeError, type JSONValue } from "../../src/merge/types";
import {
	createOrdOverlay,
	createOverlayPatch,
	loadJsonFixture,
...
test("applies JSONPath overlay example to OpenAPI metadata example", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/openapi-astronomy-api-jsonpath.overlay.json",
	);
...
// 	const overlay = await loadJsonFixture<ORDOverlay>(
// 		"examples/ord-document-1.overlay.json",
// 	);
// 	const target = await loadJsonFixture<JSONValue>(
...
// 		(entry) =>
// 			entry.ordId === "sap.foo:eventResource:BillingDocumentEvents:v1",
// 	);
//
...
test("applies operation selector (OpenAPI) overlay example to OpenAPI metadata example", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/openapi-astronomy-api.overlay.json",
	);
	const target = await loadJsonFixture<JSONValue>(
		"tests/fixtures/targets/astronomy-v1.oas3.json",
	);
...
test("applies operation selector (A2A) overlay example to A2A Agent Card example", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/a2a-dispute-agent.overlay.json",
	);
	const target = await loadJsonFixture<JSONValue>(
		"tests/fixtures/targets/DisputeResolutionAgentcard.json",
	);
...
test("applies entityType + propertyType selectors (CSN Interop) overlay example to airline CSN document", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/csn-interop-airline.overlay.json",
	);
	const target = await loadJsonFixture<JSONValue>(
		"tests/fixtures/airline.csn-interop.json",
	);
...
test("applies entityType + propertyType + operation selectors (CSDL JSON) to OData CSDL JSON document", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/edmx-example-service.overlay.json",
	);
	const target = await loadJsonFixture<JSONValue>(
		"tests/fixtures/ExampleService.csdl.json",
	);
...
test("applies entityType + propertyType + operation selectors (EDMX) to OData EDMX XML document", async () => {
	...
	const xmlInput = await loadTextFixture(
		"tests/fixtures/ExampleService.edmx.xml",
	);
...
test("applies business-partner EDMX overlay: entityType and propertyType patches with various annotation shapes", async () => {
	const overlay = await loadJsonFixture<ORDOverlay>(
		"examples/edmx-business-partner.overlay.json",
	);
	const xmlInput = await loadTextFixture(
...
// ─── COV-1: EDMX malformed / missing-structure error handling ─────────────────

test("applyOverlayToEdmxDocument throws OverlayMergeError when root has no edmx:Edmx element", () => {
	const overlay = createOrdOverlay({
		patches: [
			createOverlayPatch({
				selector: { entityType: "Customer" } as never,
	...
	assert.throws(
		() =>
			applyOverlayToEdmxDocument("<Root><Child /></Root>", overlay, {
				validateOverlaySemantics: false,
			}),
		OverlayMergeError,
	);
...
test("applyOverlayToEdmxDocument throws OverlayMergeError when edmx:DataServices has no Schema", () => {
	const emptyEdmx = `<?xml version="1.0" encoding="utf-8"?>
<edmx:Edmx Version="4.0" xmlns:edmx="http://docs.oasis-open.org/odata/ns/edmx">
  <edmx:DataServices />
</edmx:Edmx>`;
	...
	assert.throws(
		() =>
			applyOverlayToEdmxDocument(emptyEdmx, overlay, {
				validateOverlaySemantics: false,
			}),
		(err: unknown) => {
			assert.ok(err instanceof OverlayMergeError);
			assert.match(err.message, /Schema elements/);
			return true;
		},
	);
});

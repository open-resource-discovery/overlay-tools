...
test("cli --dry-run validates without applying changes", async () => {
	...
	assert.equal(stdout, "");
	// Dry run should emit validation report to stderr
	assert.match(stderr, /Dry-Run Validation Report/);
	assert.match(stderr, /Status: VALID/);
	assert.match(stderr, /Patches: 1/);
});

test("cli exits with code 1 and reports error when overlay file does not exist", async () => {
	try {
		await execFileAsync(process.execPath, [
			cliPath,
			"--overlay",

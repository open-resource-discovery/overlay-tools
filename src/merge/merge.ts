import type { ORDOverlay, OverlayPatch } from "../spec";
...
function deepMerge(base: JSONValue, incoming: JSONValue): JSONValue {
	...
	const baseType =
		base === null ? "null" : Array.isArray(base) ? "array" : typeof base;
	const incomingType =
		incoming === null
			? "null"
	...
	if (baseType !== incomingType) {
		console.warn(
			`[overlay-merge] Warning: Type mismatch in merge: replacing ${baseType} with ${incomingType}.`,
		);
	}

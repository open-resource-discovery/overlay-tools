...
export function cloneJSONValue<T>(value: T): T {
	try {
		const cloneFn = (globalThis as { structuredClone?: <U>(input: U) => U })
			.structuredClone;
		...
		throw new OverlayMergeError(
			`Cannot clone value: likely a circular reference or non-serializable data. ${String(err)}`,
		);
	}
}

export function matchesOverlayTarget(
	overlay: ORDOverlay,
	context: OverlayMergeContext,
): boolean {
	...
	if (
		context.ordId !== undefined &&
		target.ordId !== undefined &&
		context.ordId !== target.ordId
	) {
		return false;
	}
